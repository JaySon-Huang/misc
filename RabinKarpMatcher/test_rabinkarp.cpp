...
public:
    static void SetUpTestCase() {
        const static unsigned int NUM_KEYWORD = 8;
...
#endif
    MatcherTest::SetUpTestCase();
    for (auto iter: MatcherTest::sentences)
    {
        for (int i=0; i!=TEST_ROUND; ++i){
            MatcherTest::rk_matcher->match(iter.m_sentence, NULL, NULL);
        }
    }
}
