...
int token_in(const set<enum Kind> *pset, const struct parse_state_t *pstate) {
...
int token_match(const set<enum Kind> *pset, struct parse_state_t *pstate) {
    if (pset->find(pstate->cur_token.kind) != pset->end()) {
...
int token_match_one(const enum Kind kind, struct parse_state_t *pstate) {
...
void init_parser(struct parse_state_t *pstate,
 ...
 */
struct syntax_tree_node_t *parse_program(struct parse_state_t *pstate) {
    SymbolTable symbol_table;
    parse_declarations(&symbol_table, pstate);  // 分析变量的声明
    printf("Symbol Table:\n");
    for (std::set<struct symbol_t>::iterator iter =
             symbol_table.m_table.begin();
         iter != symbol_table.m_table.end(); ++iter) {
        printf("ValType: ");
 ...
 */
void parse_declarations(SymbolTable *ptable, struct parse_state_t *pstate) {
    if (DEBUG) printf("parsing declarations...\n");
    // type-specifier -> int|bool|string
    while (pstate->cur_token.kind == TK_INT ||
        ...
        if (DEBUG) token_pair_print(&type);

        parse_state_next_token(pstate);
        // varlist -> identifier[, varlist]
        while (1) {
            token_pair_copy(&id, &pstate->cur_token);
            if (DEBUG) token_pair_print(&id);
            if (token_match_one(TK_ID, pstate)) {
                ...
                if (DEBUG) printf("new identifier: %s\n", id.value.c_str());
            }
    ...
    if (DEBUG) printf("End of declarations\n");
}

/* stmt-sequenct -> statement[; stmt-sequence]
 */
struct syntax_tree_node_t *parse_stmt_sequence(struct parse_state_t *pstate) {
    if (DEBUG) printf("Parsing statement sequence..\n");
    ...
    if (DEBUG) printf("End of statement sequence.\n");
    return t;
}
 ...
 */
struct syntax_tree_node_t *parse_statement(struct parse_state_t *pstate) {
    if (DEBUG) printf("Parsing one statement.\n");
    struct syntax_tree_node_t *t = NULL;
    switch (pstate->cur_token.kind) {
        case TK_IF:
            if (DEBUG) printf("statement -> if-stmt\n");
            t = parse_if_stmt(pstate);
            break;
        case TK_WHILE:
            if (DEBUG) printf("statement -> while-stmt\n");
            t = parse_while_stmt(pstate);
            break;
        case TK_REPEAT:
            if (DEBUG) printf("statement -> repeat-stmt\n");
            t = parse_repeat_stmt(pstate);
            break;
        case TK_ID:
            if (DEBUG) printf("statement -> assign-stmt\n");
            t = parse_assign_stmt(pstate);
            break;
        case TK_READ:
            if (DEBUG) printf("statement -> read-stmt\n");
            t = parse_read_stmt(pstate);
            break;
        case TK_WRITE:
            if (DEBUG) printf("statement -> write-stmt\n");
    ...
    if (DEBUG) printf("End of one statement.\n\n");
    return t;
}
 ...
 */
struct syntax_tree_node_t *parse_if_stmt(struct parse_state_t *pstate) {
    ...
    if (DEBUG) printf("End of if-stmt\n");
    return t;
}

/* while-stmt -> while logical-or-exp do stmt-sequence end
 */
struct syntax_tree_node_t *parse_while_stmt(struct parse_state_t *pstate) {
    ...
    token_match_one(TK_END, pstate);
    if (DEBUG) printf("End of while-stmt\n");
    return t;
}

/* repeat-stmt -> repeat stmt-sequence until logical-or-exp
 */
struct syntax_tree_node_t *parse_repeat_stmt(struct parse_state_t *pstate) {
    ...
    if (DEBUG) printf("End of repeat-stmt\n");
    return t;
}

/* read-stmt -> read identifier
 */
struct syntax_tree_node_t *parse_read_stmt(struct parse_state_t *pstate) {
    ...
    if (DEBUG) printf("End of read-stmt\n");
    return t;
}

/* write-stmt -> write logical-or-exp
 */
struct syntax_tree_node_t *parse_write_stmt(struct parse_state_t *pstate) {
    struct syntax_tree_node_t *t = new_stmt_node(STMT_WRITE);
    token_match_one(TK_WRITE, pstate);
    if (t != NULL) {
        t->child[0] = parse_exp(pstate);
    }
    if (DEBUG) printf("End of write-stmt\n");
    return t;
}

/* assign-stmt -> identifier := logical-or-exp
 */
struct syntax_tree_node_t *parse_assign_stmt(struct parse_state_t *pstate) {
    if (DEBUG) printf("assign-stmt -> identifier := logical-or-exp\n");
    ...
    if (DEBUG) printf("End of assign-stmt\n");
    return t;
}

/* bool-exp -> logical-or-exp
 */
struct syntax_tree_node_t *parse_bool_exp(struct parse_state_t *pstate) {
    if (DEBUG) printf("Parsing Bool expression\n");
    struct syntax_tree_node_t *t = parse_or_exp(pstate);
    if (DEBUG) printf("End of Bool expression\n");
    return t;
}

/* logical-or-exp -> logical-and-exp[or logical-or-exp]
 */
struct syntax_tree_node_t *parse_or_exp(struct parse_state_t *pstate) {
 ...
 */
struct syntax_tree_node_t *parse_and_exp(struct parse_state_t *pstate) {
...
/*
 */
struct syntax_tree_node_t *parse_not_exp(struct parse_state_t *pstate) {
...
/*
 */
struct syntax_tree_node_t *parse_exp(struct parse_state_t *pstate) {
    if (DEBUG) printf("Parsing expression\n");
    struct syntax_tree_node_t *t = NULL;
    switch (pstate->cur_token.kind) {
        case TK_NUM:
        case TK_HEXNUM:
    ...
    if (DEBUG) printf("End of expression.\n");
    return t;
}

/* comparison-exp -> add-exp [comparison-op comparison-exp]
 */
struct syntax_tree_node_t *parse_compare_exp(struct parse_state_t *pstate) {
...
/* 处理 加减
 */
struct syntax_tree_node_t *parse_arithmetic_exp(struct parse_state_t *pstate) {
...
/* 处理 乘除
 */
struct syntax_tree_node_t *parse_term(struct parse_state_t *pstate) {
 ...
 */
struct syntax_tree_node_t *parse_factor(struct parse_state_t *pstate) {
    struct syntax_tree_node_t *t = NULL;
    switch (pstate->cur_token.kind) {
        case TK_NUM:
        case TK_OCTNUM:
...
/*
 */
struct syntax_tree_node_t *parse_string_exp(struct parse_state_t *pstate) {
    if (DEBUG) printf("expression string\n");
