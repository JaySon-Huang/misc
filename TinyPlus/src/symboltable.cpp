...
void SymbolTable::update(const token_pair_t *token, enum ObjectType obj_type,
...
const struct symbol_t *SymbolTable::find(const string *name) {
    struct symbol_t symbol;
...
struct symbol_t *SymbolTable::del(const string *name) {
    struct symbol_t symbol;
    symbol.token.value = *name;
    const set<struct symbol_t>::iterator found = m_table.find(symbol);
    if (found == m_table.end()) {
