...
static void gen_stmt(struct syntax_tree_node_t *root,
...
static void gen_exp(
    ...
    vector<struct middle_code_t> *pcode_vec);
*/
static void gen_rval(struct syntax_tree_node_t *root,
                     struct middle_code_t *pfa_code,
                     struct code_gen_state_t *pstate,
                     vector<struct middle_code_t> *pcode_vec);
...
void code_init_tmp(struct code_gen_state_t *pstate) { pstate->tmp_index = 0; }

void code_print(struct middle_code_t *pcode) {
    if (pcode->islabel) {
        printf("%s\n", pcode->store.c_str());
    } else {
        printf("%4d :(%3s, %4s, %4s, %5s)\n", pcode->lineno, pcode->op.c_str(),
               pcode->larg.c_str(), pcode->rarg.c_str(), pcode->store.c_str());
    }
}

static void code_fill_arg(struct middle_code_t *pcode, string *arg) {
    if (pcode->larg == "") {
...
static void gen_stmt(struct syntax_tree_node_t *root,
    ...
    code.islabel = 0;
    label.islabel = label2.islabel = 1;
    // label.is_label = true;
    switch (root->node_type) {
        case STMT_IF:
            /*
            ...
            */
            ...
            if (DEBUG) printf("Generating code for if.\n");
            code.op = "jnz";
        ...
        case STMT_WHILE:
            if (DEBUG) printf("Generating code for while.\n");
            // 循环的位置
            sprintf(t, "Label %2d", code_gen_next_index(pstate));
        ...
        case STMT_REPEAT:
            if (DEBUG) printf("Generating code for repeat.\n");
            sprintf(t, "Label %2d", code_gen_next_index(pstate));
            label.store.assign(t);
            pcode_vec->push_back(label);
            if (DEBUG) printf("%s\n", label.store.c_str());
        ...
        case STMT_ASSIGN:
            if (DEBUG) printf("Generating code for assign.\n");
            code.op = ":=";
        ...
        case STMT_READ:
            if (DEBUG) printf("Generating code for read.\n");
            code.op = "read";
            code.store = root->token.value;
            break;

        case STMT_WRITE:
            if (DEBUG) printf("Generating code for write.\n");
            code.op = "write";
